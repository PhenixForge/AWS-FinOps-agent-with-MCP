...
- ⏳ **Durcissement IAM différé** (`00-IAM.tf`) — trust policy du rôle Gateway sans condition `aws:SourceAccount`/`aws:SourceArn` (prévention "confused deputy"), volontairement omise tant que l'ARN réel du Gateway n'existe pas (recommandation AWS elle-même). À ajouter juste après le premier `terraform apply`. Détail dans la section Sécurité du [README.md](README.md#sécurité).
...
## Retour d'expérience : classification de risque EU AI Act (19 septembre 2026)

Exercice volontaire, pas une obligation identifiée pour ce projet — l'idée est de documenter le raisonnement de classification plutôt que de laisser un flou ou d'affirmer une conformité de façade. Résultat en fin de [README.md](README.md#conformité-eu-ai-act).

**Pourquoi maintenant** : mon cutoff de connaissances datait de janvier 2026 sur ce sujet, et le calendrier de l'AI Act a bougé depuis — le *Digital Omnibus* (adopté par le Parlement européen le 16 juin 2026, approbation finale du Conseil le 29 juin 2026) a reporté l'échéance des obligations "haut risque" de l'Annexe III du 2 août 2026 au 2 décembre 2027 (Annexe I, systèmes intégrés à des produits déjà couverts par la législation produit UE : 2 août 2028). Seules restent inchangées : les pratiques interdites (Art. 5, en vigueur depuis février 2025), les obligations des fournisseurs de modèles GPAI (Art. 53+, en vigueur depuis août 2025), et les obligations de transparence (Art. 50, en vigueur depuis le 2 août 2026 — donc actives depuis quelques semaines au moment d'écrire ceci).

**Conclusion** : ce projet ne semble déclencher aucune obligation substantielle.
- Pas "haut risque" au sens de l'Annexe III (infrastructure critique) : cette catégorie vise un système utilisé comme *composant de sécurité* dans la gestion/opération de l'infrastructure — cet agent est strictement en lecture (Cost Explorer, CloudWatch, EC2 `Describe*`), un outil consultatif, jamais un composant de gestion ou d'opération.
- Les obligations GPAI pèsent sur le fournisseur du modèle (Bedrock/Anthropic), pas sur un déployeur qui l'appelle via API.
- L'obligation de transparence (Art. 50 — informer l'utilisateur qu'il interagit avec une IA) est probablement couverte par l'exemption "évident selon le contexte" : les seuls clients de démo (Claude Code CLI, connecteur MCP claude.ai) sont eux-mêmes des interfaces IA explicites.

**Prudence** : ceci reste une lecture personnelle du texte à un instant T, pas un avis juridique, et le calendrier a déjà bougé une fois cette année — à revérifier avant toute décision qui en dépendrait réellement (usage au-delà d'un projet personnel, par exemple).
